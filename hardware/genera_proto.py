...
{agujeros}
          ...
          physics Physics {{ density -1 mass {f(mr.MASA_RUEDA)} centerOfMass [ 0 0 0 ] inertiaMatrix [ {trans:.3e} {eje:.3e} {trans:.3e}, 0 0 0 ] }}
        }}
      }}"""
...
def tof(n, x, y, yaw):
    return f"""      DistanceSensor {{
        ...
        lookupTable [ 0 0 0.004, 1.2 1200 0.02 ]
        children [
...
def qtr(n, x, y):
    ...
    return f"""      DistanceSensor {{
        ...
        lookupTable [ 0 1000 0.0, 0.012 900 0.01, 0.016 20 0.03 ]
        children [
...
#
#  Motores Pololu #3063 (50:1 HPCB) a 7.4 V: {mr.MAX_VEL:.1f} rad/s, {mr.MAX_TORQUE:.4f} N*m.
...
#
#  Distancia: 5 x VL53L0X con UN rayo, como tests/harness.c. Con varios rayos
#  Webots promedia como 1200 los que no tocan nada y los inferiores ven el
#  suelo del dohyo a ~0.5 m (rival fantasma a ~750 mm).
#  Linea: 4 x QTR-1A. Webots divide la distancia (~5.7 mm) por la
#  reflectancia antes de la tabla: blanco -> ~935, negro -> 20. La pala del
#  rival metida debajo tambien se lee como blanco, igual que en el robot real.
...
{chr(10).join(tof(*d) for d in mr.DIST)}
...
- Motor en la rueda: {mr.MAX_VEL:.1f} rad/s en vacío y {mr.MAX_TORQUE:.4f} N·m de bloqueo a 7,4 V;
  unos {mr.MAX_VEL * 0.021:.2f} m/s y {2 * mr.MAX_TORQUE / 0.021:.1f} N de empuje con las dos ruedas.

## Limitaciones conocidas del modelo

- Webots aplica el par máximo constante hasta la velocidad máxima; un motor de
  continua real pierde par a medida que gira más rápido.
- No se modela la inercia del rotor reflejada a través de la reductora.
